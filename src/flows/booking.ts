...
export class BookingFlow {
  ...
  private async showConfirmation(userId: string): Promise<BotResponse> {
    ...
    // Crear resumen
    const tempReservation = reservationService.createReservation(
  ...
  private completeReservation(userId: string): BotResponse {
    contextService.updateFlow(userId, ConversationFlow.COMPLETED);

    const response = `✅ ¡Tu reserva ha sido confirmada exitosamente!

Pronto recibirás un email con los detalles completos de tu reserva y número de confirmación.

${PROMPTS.FAREWELL}`;

    contextService.addMessage(userId, 'assistant', response);
    logger.info('Reserva completada', { userId });
